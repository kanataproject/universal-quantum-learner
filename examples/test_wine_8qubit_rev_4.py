...
print("======================================================")
print("  完全ハイブリッド量子AI - [ワイン13次元：第3版 ハイブリッド]")
print("======================================================\n")
    ...
    def _quantum_circuit(self, inputs, q_weights):
        for layer in range(self.num_layers):
            for i in range(self.n_qubits):
                ...
                if layer == 0:
                    # 【第3版：究極ハイブリッドエンコード】
                    # inputsはPCAで「直交化(無相関化)された13次元」。
                    # 干渉しない純粋な特徴量なので、RYとRZに同時に乗せてもノイズにならない。
                    
                    # 上位8次元を全QubitのRYへ
                    qml.RY(inputs[i] + q_weights[weight_idx], wires=i)
                    
                    # 残りの5次元(第9〜第13主成分)をQubit 0〜4のRZへ
                    if i < 5:
...
if __name__ == "__main__":
    ...
    # 【第3版のキモ】 PCAで次元を「減らす」のではなく、「直交化(無相関化)」するために13次元そのまま保持する
