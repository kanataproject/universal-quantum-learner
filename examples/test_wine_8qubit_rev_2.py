...
from sklearn.decomposition import PCA # 追加：PCA圧縮用

print("======================================================")
print("  完全ハイブリッド量子AI - [ワイン13次元：第1版 PCAエンコード]")
print("======================================================\n")

class WineQuantumClassifier8Qubit_V1:
    ...
    def _quantum_circuit(self, inputs, q_weights):
        for layer in range(self.num_layers):
            for i in range(self.n_qubits):
                ...
                if layer == 0:
                    # 【第1版のキモ】
                    # 古典側ですでに綺麗な8次元に圧縮されているため、
                    # 足し算などの小細工をせず、1 Qubitに対して1次元をストレートに割り当てる
...
if __name__ == "__main__":
    ...
    # 【第1版のキモ】PCAで13次元の情報を損失を最小限に抑えつつ8次元に圧縮
    ...
    # PCA圧縮済みの8次元データをフォーマット
    formatted_data = [(np.array(X_pca[i], requires_grad=False), y_binary[i]) for i in range(len(X_pca))]
    train_data = formatted_data[:140]
    test_data = formatted_data[140:]

    classifier = WineQuantumClassifier8Qubit_V1(n_qubits=8, num_layers=4)
