...
# ======================================================
# 量子AI クラス本体 (6Qubit・4層・古典16ノード 適材適所最適化版)
# ======================================================
class BreastCancerQuantumClassifier6Qubit:
    def __init__(self, n_qubits=6, num_layers=4): 
        ...
        # 量子出力は 12次元 (Zが6個、Xが6個)
        # 隠れ層16ノード: 16 * 12 + 16(bias) = 208
        # 出力層: 16 * 1 + 1(bias) = 17
        # 古典層合計: 225
        self.c_param_count = 225 
    ...
    def _quantum_circuit(self, inputs, q_weights):
        for layer in range(self.num_layers):
            for i in range(self.n_qubits):
                ...
                if layer == 0:
                    # 30次元データを6Qubitに効率良く分散エンコード (各Qubitに5次元ずつマッピング)
                    # i=0: 0,6,12,18,24 | i=1: 1,7,13,19,25 ...
                    qml.RY(inputs[i] + q_weights[weight_idx], wires=i)
                    qml.RZ(inputs[i + 6], wires=i)
                    qml.RX(inputs[i + 12], wires=i)
                    qml.RY(inputs[i + 18], wires=i)
                    qml.RZ(inputs[i + 24], wires=i)
                else:
    ...
    def _neural_network(self, inputs, weights, step_size):
        ...
        for i in range(16): # 16ノード
            idx = offset + (i * 13) # 12入力 + 1バイアス
            h_in = sum(weights[idx + j] * translated[j] for j in range(12)) + weights[idx + 12]
...
def run_parallel_hunter(step_size, X_train, y_train, X_test, y_test, initial_weights, epochs, log_interval, shared_dict, timestamp):
    ...
    log_filename = f"logs/cancer_hunter_6Q_16N_{timestamp}_step_{step_size}.log"
    with open(log_filename, "w", encoding="utf-8") as f:
        f.write(f"======================================================\n")
        f.write(f" 乳がん30次元対応(6Qubit最適化 / 16ノード) - 本格検証版\n")
        ...
        classifier = BreastCancerQuantumClassifier6Qubit(n_qubits=6, num_layers=4)
...
def run_control_panel(X_train, y_train, X_test, y_test, step_candidates, initial_weights, log_interval, shared_dict, timestamp):
    ...
    pygame.display.set_caption("30D Cancer Data (6Qubit/16Nodes) High-Res Hunt")
    ...
    BG_COLOR = (15, 20, 30) # 6Qubit版として少し色調をサイバーブルー寄りに
    TEXT_COLOR = (220, 230, 230)
    COLOR_BTN_IDLE = (50, 60, 80)
    COLOR_BTN_HOVER = (70, 80, 100)
    COLOR_START_IDLE = (60, 140, 140)
    COLOR_START_HOVER = (80, 170, 170)
    
    COLOR_RUNNING = (110, 210, 110)
    COLOR_INIT = (130, 120, 120)
    COLOR_KILL_LOOP = (220, 140, 40)
    COLOR_KILL_STAG = (210, 80, 80)
    COLOR_GOAL = (150, 220, 255)
    ...
    while running:
        ...
        if ui_state == "SETUP":
            title = font_xl.render("6-QUBIT / 16-NODE HIGH-RES HUNT", True, TEXT_COLOR)
            screen.blit(title, (WIDTH//2 - title.get_width()//2, 100))
            epoch_label = font_l.render("Max Target Epochs", True, (150, 160, 160))
            ...
            start_txt = font_xl.render("LAUNCH 6Q-HUNT", True, (255, 255, 255))
            ...
            title = font_l.render("6Qubit High-Res Hunter Dashboard", True, (180, 190, 190))
            ...
            for step in step_candidates:
                ...
                pygame.draw.rect(screen, (30, 35, 45), panel_rect, border_radius=6)
                ...
                info_text1 = font_s.render(f"Cost: {state['cost']:.5f} | Best Acc: {best_acc*100:.2f}% (Ep {best_ep})", True, (200, 210, 210))
                info_text2 = font_s.render(f"Stagnation: {stag}/3", True, COLOR_KILL_STAG if stag > 0 else (150, 160, 160))
                ...
                pygame.draw.rect(screen, (50, 55, 65), bar_bg, border_radius=4)
            ...
            if active_processes == 0 and len(shared_dict) == len(step_candidates):
                footer_txt = "6Q-Lanes finished hunting! Let's check the adaptation score."
                footer_color = COLOR_GOAL
            else:
                footer_txt = f"Active Hunters: {active_processes} / {len(step_candidates)} (Lightweight Simulation)"
...
if __name__ == "__main__":
    ...
    # 10Qubitの時の傾向から、中堅〜荒めを中心にシフトを再配備
    step_candidates = [1, 0.5, 0.1, 0.01, 0.001, 0.000001]
    
    np.random.seed(42)
    # 6Qubit量子層(24) + 古典16ノード(225) = 249
    initial_weights = list(np.random.uniform(-0.5, 0.5, size=249))
    ...
    print("\n[INFO] 30次元 Breast Cancer (6Qubit / 16ノード / 空間最適化版) を起動します...")
