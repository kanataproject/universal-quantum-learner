...
print("======================================================")
print("  完全ハイブリッド量子AI - [ワイン13次元：8 Qubit リベンジマッチ]")
print("======================================================\n")

class WineQuantumClassifier8Qubit:
    ...
    def _quantum_circuit(self, inputs, q_weights):
        for layer in range(self.num_layers):
            for i in range(self.n_qubits):
                ...
                if layer == 0:
                    # 【8Qubit用高次元エンコード】13次元を8つのqubitにバランスよく圧縮
                    if i < 5:
                        # 0,1,2,3,4番目のqubitには2つの次元をブレンド
                        val = inputs[i] + inputs[i + 5]
                    else:
                        # 5,6,7番目のqubitには3つの次元をブレンド (i=5 -> 5, 10, idxオーバー防止)
                        idx2 = i + 5
                        idx3 = i + 5 + 3
                        val = inputs[i] + inputs[idx2]
                        if idx3 < len(inputs):
                            val += inputs[idx3]
    ...
    def fit(self, train_data, epochs=150, log_interval=15):
        ...
        print(f"=> 最最適粒度を【 {self.best_step} 】にロックオン。\n")

        print("=== [Phase 2] ワイン実データ メイン学習フェーズ（8 Qubitスケール版） ===")
...
if __name__ == "__main__":
    ...
    # 8 Qubitモデルで実行（計算グラフが大きくなるので実行時間は数十秒かかります）
    classifier = WineQuantumClassifier8Qubit(n_qubits=8, num_layers=4)
