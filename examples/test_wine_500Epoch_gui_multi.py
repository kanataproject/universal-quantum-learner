...
def run_parallel_hunter(step_size, X_train, y_train, X_test, y_test, initial_weights, epochs, log_interval, shared_dict, timestamp):
    ...
    log_filename = f"logs/hunter_{timestamp}_step_{step_size}.log"
    with open(log_filename, "w", encoding="utf-8") as f:
        f.write(f"======================================================\n")
        f.write(f" 量子AI 最終生産仕様 - 完全自律型マルチハンター\n")
        f.write(f" 担当粒度 (Step Size) : {step_size}\n")
        f.write(f" 最大エポック (Cap)   : {epochs}\n")
        f.write(f" 停止条件1: Costの低下幅が 0.0001 以下で停止 (LOOP判定)\n")
        f.write(f" 停止条件2: Test Accが 2回連続未更新で停止 (過学習ガード)\n")
        f.write(f"======================================================\n\n")
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % log_interval == 0:
                ...
                if stagnation_count >= PATIENCE:
                    ...
                    f.write(f"=> 🏁 [MISSION COMPLETE] 採用モデル: Epoch {best_epoch} (Acc: {best_accuracy*100:.2f}%)\n")
                    return
            
                # 【Kill判定1】ウロウロ・底(LOOP判定) - 15エポック毎のチェックに変更
                if (prev_cost - cost) <= 0.0001:
                    # 死に際テスト
                    final_acc = classifier.evaluate(test_data, step_size)
                    ...
                    shared_dict[step_size] = state
                    f.write(f"\n=> ⚠️ [KILL] 15エポック間のCost減少幅の停滞(LOOP限界)を検知し自動停止。\n")
                    f.write(f"=> 🏁 [MISSION COMPLETE] 採用モデル: Epoch {best_epoch} (Acc: {best_accuracy*100:.2f}%)\n")
                    return
                    
                prev_cost = float(cost) # 15エポック前のCostとして更新
...
def run_control_panel(X_train, y_train, X_test, y_test, step_candidates, initial_weights, log_interval, shared_dict, timestamp):
    ...
    pygame.display.set_caption("Quantum AI Parallel Hunter Dashboard")
    ...
    COLOR_START_IDLE = (50, 150, 180)
    COLOR_START_HOVER = (70, 180, 210)
    
    COLOR_RUNNING = (50, 210, 110)
    COLOR_INIT = (120, 120, 130)
    ...
    while running:
        ...
        if ui_state == "SETUP":
            title = font_xl.render("MULTI-LANE PARALLEL HUNTER", True, TEXT_COLOR)
            screen.blit(title, (WIDTH//2 - title.get_width()//2, 100))
            epoch_label = font_l.render("Max Target Epochs", True, (150, 150, 160))
            ...
            start_txt = font_xl.render("LAUNCH MULTI-HUNT", True, (255, 255, 255))
            ...
            title = font_l.render("Parallel Autonomous Hunter Dashboard", True, (180, 180, 190))
            ...
            for step in step_candidates:
                ...
                pygame.draw.rect(screen, (30, 30, 40), panel_rect, border_radius=6)
                ...
                info_text2 = font_s.render(f"Stagnation: {stag}/2", True, COLOR_KILL_STAG if stag > 0 else (150, 150, 160))
                ...
                pygame.draw.rect(screen, (50, 50, 60), bar_bg, border_radius=4)
            ...
            if active_processes == 0 and len(shared_dict) == len(step_candidates):
                footer_txt = "All lanes finished hunting! Check logs for the global best model."
...
if __name__ == "__main__":
    ...
    # 【最終版】探索する粒度のリスト
    step_candidates = [1.0, 0.5, 0.1, 0.05, 0.01, 0.001]
