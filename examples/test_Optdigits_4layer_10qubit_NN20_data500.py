        ...
        if ui_state == "SETUP":
            ...
            if active_processes == 0 and len(shared_dict) == len(step_candidates):
                ...
                footer_txt   = (f"Active Hunters: {active_processes} / {len(step_candidates)} "
                                f"| {config['data']['input_dimensions']}D / {output_classes}-Class / {nodes} Nodes")
...
if __name__ == "__main__":
    ...
    try:
        ...
        config = {
            ...
            "classical_model": {"hidden_nodes": 20},
            "hunter_settings": {
                "max_epochs": 1000,
                "cost_reversal_limit": 3,
                "log_interval": 5,
                "learning_rate": 0.05,
    ...
    print(f"[INFO] 現在の学習率設定 (Learning Rate): {config['hunter_settings']['learning_rate']}")
    print(f"[INFO] 古典層ノード数 (Hidden Nodes): {config['classical_model']['hidden_nodes']}")
    print(f"[INFO] 探索粒度候補 (Step Candidates): {config['hunter_settings']['step_candidates']}")
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        # 学習データを500件、テストを100件へ増量
        X_scaled, y, train_size=500, test_size=100,
