...
print("======================================================")
print("  完全ハイブリッド量子AI - [汎用性検証：ワイン13次元データ]")
print("======================================================\n")

class WineQuantumClassifier:
    def __init__(self, n_qubits=4, num_layers=4, step_candidates=None):
        ...
        # 4 qubits * 4 layers = 16量子パラメータ
        self.q_param_count = self.n_qubits * self.num_layers
        # 古典NN: 8入力(4量子ビットのZ/X測定) -> 4隠れノード -> 1出力
        # (8*4 + 4) + (4*1 + 1) = 41古典パラメータ
        self.c_param_count = 41
    ...
    def _quantum_circuit(self, inputs, q_weights):
        for layer in range(self.num_layers):
            for i in range(self.n_qubits):
                ...
                if layer == 0:
                    # 【高次元ハック】13次元の入力を4つのqubitにインデックスを循環させてエンコード
                    # 各qubitの初期回転に、複数の次元の特徴量をブレンドして乗せる
                    val = inputs[i] + inputs[i + 4] + inputs[i + 8] if (i + 8) < len(inputs) else inputs[i] + inputs[i + 4]
                    qml.RY(val + q_weights[weight_idx], wires=i)
                else:
                    # 第2層以降は純粋な重みのみ（Data Re-uploading排除を維持）
    ...
    def fit(self, train_data, epochs=150, log_interval=15):
        ...
        print("=== [Phase 2] ワイン実データ メイン学習フェーズ（150Epoch） ===")
...
if __name__ == "__main__":
    # 1. ワインデータのロードと前処理
    wine = load_wine()
    X = wine.data  # 13次元の特徴量
    y = wine.target
    
    # ターゲットを「クラス1（アルコール度数等の中間グループ）かそれ以外か」に加工
    y_binary = np.array([1.0 if label == 1 else 0.0 for label in y])
    
    # 特徴量の標準化
    X_scaled = (X - np.mean(X, axis=0)) / np.std(X, axis=0)
    
    # シャッフルして訓練データ(140件)とテストデータ(38件)に分割
    np.random.seed(42)
    ...
    # 2. 実行
    classifier = WineQuantumClassifier(n_qubits=4, num_layers=4)
